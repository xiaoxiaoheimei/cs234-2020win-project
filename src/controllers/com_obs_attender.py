...
class ComObsAttender(nn.Module):
   ...
   def extEnvInfo(self, env):
       info = env.get_env_info()
       self.n_agents = info['n_agents']
       self.obs_size = info['obs_shape']
       self.n_enemies = self.env.n_enemies
       self.nf_al = 4 + self.env.unit_type_bits
       self.nf_en = 4 + self.env.unit_type_bits

       if self.env.obs_all_health:
          self.nf_al += 1 + self.env.shield_bits_ally
          self.nf_en += 1 + self.env.shield_bits_enemy

       if self.env.obs_last_action:
          self.nf_al += self.env.n_actions

       self.nf_own = self.env.unit_type_bits
       if self.env.obs_own_health:
          self.nf_own += 1 + self.env.shield_bits_ally

       self.move_feats_len = self.env.n_actions_move
       if self.env.obs_pathing_grid:
          self.move_feats_len += self.env.n_obs_pathing
       if self.env.obs_terrain_height:
          self.move_feats_len += self.env.n_obs_height
